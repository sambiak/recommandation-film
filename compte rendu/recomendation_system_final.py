import numpy as np
import math
from movielens import tableau_des_notes, tableau_bricole_de_l_lignes_et_c_colonnes
...
def grad_J_par_rapport_a_x_j(Y, x_j, j, theta):
    """
    :param Y: tableau de notes avec utilisateurs en lignes et films en colonnes
    :param x_j: ligne j de X par rapport a laquelle nous calculons les derivees
    partielles de la fonction de cout (correspond aux caracteristiques du film j)
    :param j: numero de la ligne de X par rapport a laquelle nous calculons les derivees
...
def grad_J_par_rapport_a_theta_i(Y, theta_i, i, X):
    """
    :param Y: tableau de notes avec utilisateurs en lignes et films en colonnes
    :param theta_i: ligne i de theta par rapport a laquelle nous calculons les derivees
    partielles de la fonction de cout
            (correspond aux caracteristiques du film j)
    :param i: numero de la ligne de theta par rapport a laquelle nous calculons les derivees
    partielles de la fonction de cout
...
def etape_du_gradient(Y, alpha_X, alpha_theta, theta, X):
    ...
    :param theta: tableau avec des lignes de profils d'utilisateurs
    :param X: tableau avec des lignes de caracteristiques de films
    :return: theta et X modifies apres une etape de descente du gradient
    """
    ...
    for i, theta_i in enumerate(theta):
        n_theta[i] = (theta_i - alpha_theta * grad_J_par_rapport_a_theta_i\
            (Y, theta_i, i, X))
...
def descente_du_gradient(Y, nb_carac, nb_etapes, alpha_X, alpha_theta):
    ...
    :param nb_etapes: nb d etapes de descente du gradient a realiser
    :param alpha_X: taux d apprentissage pour la modification de X avec la descente du gradient
    :param alpha_theta: taux d apprentissage pour la modification de theta
    avec la descente du gradient
    :return: theta et X modifies apres nb_etapes d'etapes de descente du gradient
    """
...
def descente_du_gradient_2(X, theta, Y, nb_carac, nb_etapes, alpha_X,  alpha_theta):
    """
    :param X: X fixe au depart
    :param theta: theta fixe au depart
    :param Y: tableau de notes avec utilisateurs en lignes et films en colonnes
    :param nb_carac: nombre de caracteristiques de films (nb de colonnes de theta et X)
    :param nb_etapes: nb d etapes de descente du gradient a realiser
    :param alpha_X: taux d apprentissage pour la modification de X avec la
     descente du gradient
    :param alpha_theta: taux d apprentissage pour la modification de theta avec
