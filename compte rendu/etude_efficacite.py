...
def notes_extraites_pour_validation(tableau_des_notes_entier):
    """
    :param tableau_des_notes_entier: tableau de notes avec utilisateurs
     en lignes et films en colonnes
    :return: tableau de notes avec 10% des notes enlevees(tableau de validation),
    liste indiquant les couples caracterisant les notes enlevees (utilisateur,film)
    """
    na_n = float('nan')
    ...
    for n_eme_note_de_validation in range (9922):
        ...
        while nbre_de_film_vu_par_utilisateur(tableau_des_notes_entier,numero_utilisateur)\
                < 10:
            numero_utilisateur = randrange (0,670) 
...
def notes_extraites_pour_test(tableau_des_notes_validation,tableau_des_notes_entier):
    """
    :param tableau_des_notes_validation: tableau des notes (moins 10% des notes)
    utilise pour optimiser
    :param tableau_des_notes_entier: tableau de notes avec utilisateurs en lignes et
    films en colonnes
    :return: tableau de notes avec 10% de notes enlevees (pas les memes que pour la validation),
     liste indiquant les couples caracterisant les notes enlevees (utilisateur,film)
    """
    na_n = float('nan')
    ...
    for n_eme_note_de_validation in range (9922):
        numero_utilisateur = randrange (0,670)
        while nbre_de_film_vu_par_utilisateur(tableau_des_notes_validation,numero_utilisateur)\
                < 10:
            numero_utilisateur = randrange (0,670) 
        numero_film = randrange (1,9125) 
        compteur = [0,0]
...
def ecart_quadratique(L_notes_enlevees,Y_predit,tableau_des_notes_entier):
    """
    :param L_notes_enlevees: liste de couples caracterisant les notes enlevees
    du tableau (utilisateur,film)
    :param Y_predit: tableau de notes plein avec prediction par descente du gradient
    a partir du tableau de validation
    :param tableau_des_notes_entier: tableau de notes avec utilisateurs en lignes et
    &films en colonnes (tableau de depart)
...
def nb_etapes_optimal(mini, V, X, theta, nb_car, nb_etapes, alpha_X, alpha_theta):
    """
    :param mini: liste indiquant quand est-ce qu'on obtient l'ecart minimum
    :param V: tableau de notes (moins 10%)
    :param nb_car: nb de caracteristiques associe a chaque film
    :param nb_etapes: nb d'etapes du gradient a effectuer
    ...
    :print: ecart quadratique pour un certain nb d'etapes
    :show: graphique ecart en fonction du nbhttps://apps.wifirst.net/?redirected=true_etapes
    ...
    plt.xlabel(nom_x)
    nom_fichier = "nb_car-" + str(nb_car) + "_alpha_X-" + str(alpha_X) + "_alpha_theta-" \
                  + str(alpha_theta) + ".png"
