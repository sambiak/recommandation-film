...
Au final, nous avons trouvé un taux d'erreur optimal pour $n = 10$, $\alpha_X = 0.000125$, $\alpha_\Theta = 0.0001$, $nb_etapes \in [900,950]$. La moyenne des écarts absolus entre les notes prédites et la réalité vaut alors environ $0.79$.
...
Le genre du film est peu intéressant car notre algorithme déduit des caractéristiques au film plus précise que le genre.%mal dit
...
% je pense que c'est un peu maladroit de dire qu'on a été surchargés de travail, faudrait plutot dire qu'on a preferer se concentrer sur un autre objectif ou juste ne pas se justifié de pk on la pas fait
...
\textbf{{Mots clés:}} descente du gradient, matrice, factorisation, prédiction de notes %il faut 5 mots clés , si vous avez des idées allez y
\vspace*{\stretch{1}}
