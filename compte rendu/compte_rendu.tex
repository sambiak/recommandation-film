...
nous codons une fonction tableau\_des\_notes() qui renvoie une matrice de dimensions $n_u * n_f$ qui contient les notes données par les utilisateurs aux films. Seulement, aucun utilisateur n'a noté tous les films : il y a des cases vides dans notre tableau. Nous insérons alors dans ces cases des `` NaN ''(Not a Number), type de donnée facile à traiter dans notre programme.
...
Nous créons des fonctions permettant de faire des statistiques sur nos données.\\

Nous remarquons alors que plus de 90 \% des films possèdent entre 1 et 25 notes, et que le plus souvent un film a 2 notes. Le film auquel a été attribué le plus de notes a 339 notes.

\begin{figure}[H]
  \centering
\includegraphics[scale=0.5]{hist2.png}
\caption{Films les moins notés (90\% de nos films) triés en fonction du nombre de fois qu'ils ont été notés}
\end{figure}
...
\caption{Utilisateurs classés selon le nombre de notes qu'ils ont mis}
\end{figure}
