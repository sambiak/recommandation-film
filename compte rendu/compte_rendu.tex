...
Les lignes du tableau correspondent aux utilisateurs et les colonnes aux films. Cependant, aucun utilisateur n'a noté tous les films : il y a des cases vides dans notre tableau.
...
sur "movielens". Un plus grand nombre d'utilisateurs permettrait aussi une plus grande précision. Mais est un défi : une base de données trop grande rendrait notre algorithme très long à exécuter.\\
Autre chose, certaines méthodes existent pour limiter le sur-apprentissage dont nous avons parlé et que nous n'avons pas utilisée : le faire permettrait une précision plus élevée.\\
