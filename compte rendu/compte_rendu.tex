...
\subsection{Familiarisation de la situation avec un problème plus simple}
...
Dans ce cas-ci, nous supposons qu'il existe une relation linéaire commune pour tous les films,et que la matrice X serait une matrice colonne (c'est à dire comme s'il n'y avait qu'une caractéristique).
...
Ainsi , on trouve tous les coefficients de la matrice X ce qui nous permet ensuite grace à un simple produit matriciel de déterminer n'importe quel note dans notre tableau extrait à partir de toute les autres.\\ \\ \\ \\ \\

\begin{figure}[h!]         
  \centering
  \includegraphics[scale=0.5]{tableau_extrait.png}
  \caption{Un exemple}
\end{figure}


Dans cette exemple, la descente du gradient permet de trouver les meilleurs coefficients a,b et c de la matrice X tel que :
$\left
\{
\begin{array}{c}
  3a+4,5b+3c=2 \\
...
\end{array}
\right.$
Ici, le ``='' n'est pas un égal mais un environ égal.
Une fois qu'on à trouver grace à la descente du gradient les coefficients les plus adaptés, on trouve la note manquante qui est égal à 3a+5b+3,5c.
Cependant, cette situation n'est pas du tout applicable dans la réalité: tous les utilisateurs n'auront jamais vu tous les films.
