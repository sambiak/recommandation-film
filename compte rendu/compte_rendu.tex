...
Nous pouvons ainsi trouver les films les plus en accord avec ses goûts en regardant les plus hautes notes prédites.\\
...
nous codons une fonction tableau\_des\_notes() qui renvoie une matrice de dimensions $n_u * n_f$ qui contient les notes données par les utilisateurs aux films. Les lignes du tableau correspondent aux utilisateurs et les colonnes aux films. Seulement, aucun utilisateur n'a noté tous les films : il y a des cases vides dans notre tableau. Nous insérons alors dans ces cases des ``NaN''(Not a Number), type de donnée facile à traiter dans notre programme.
...
Le film auquel a été attribué le plus de notes a 339 notes.\\
Nous voyons aussi que chaque utilisateur a noté 148 films en moyenne.

\begin{figure}[H]
  \centering
\includegraphics[scale=0.5]{hist1.png}
\caption{Répartition des utilisateurs selon le nombre de notes qu'ils ont mis}
\end{figure}
