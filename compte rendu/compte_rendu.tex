...
%Il reste à faire:
%changer les titres
%completer l algo de guillaume
%mettre 2 alphas differents dans les algo et dans les explications de la descente du gradient en general
%Les preuves en annexe sont pas du tout rigoureuses : poser A, x, b, etc.
%ajouter petites phrases d'intro au debut de chaque annexe
%Un sommaire
%une conclusion
%mettre le code en annexe
%inclure graphiques et commenter ce qu'on obtient et donner des valeurs du taux d'erreur du nb d etape et du nb de carac dans la partie de Yassine
%mettre des NaN dans la figure du tableau des notes Y -> fait !!
%faire une belle page de présentation (Rania répond pas)
%Ajouter des figures dans la partie d'anthony
...
%compléter l'algo et mettre deux taux d'apprentissage différents
...
\Repeter{convergence de la valeur $f(x_1, x_2)$}{Affecter à $(x_1, x_2)$ la valeur ($x_1 - \alpha \frac{\partial f}{\partial x_{1}}(x_1, x_2), x_2 - \alpha \frac{\partial f}{\partial x_{2}}(x_1, x_2))$\;}
...
\begin{algorithm}
\Donnees{a mettre}  
\Res{X et $\Theta$ tels que J(X, $\Theta$) soit un minimum de J}
\PourTous{j $\in$ \{1, 2, ..., $n_f$\}}{Affecter à $x_{j}$ la valeur $x_{j}-\alpha \nabla_{x_{j}}J(X, \Theta)$\;}
\PourTous{i $\in$ \{1, 2, ..., $n_u$\}}{Affecter à $\Theta_{i}$ la valeur $\Theta_{i} - \alpha \nabla_{\theta_{i}} J(X, \Theta)$\;}
...
Si $X$ est un vecteur de dimension $w$ et $f$ une fonction qui depend de $X$ %j'attive pas a bien expliquer que f peut dependre
%de plein de variables mais que X est juste l'un d'eux 
on $\nabla_{X} f =
\begin{pmatrix}
